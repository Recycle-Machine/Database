...
class Profile(Resource):
    ...
    def post(self):
        _id = str(database.db.Recycling.insert_one({
    ...
    def put(self, by, data):
        ...
        database.db.Recycling.update_one({'_id':ObjectId(response['_id'])},
        {'$set':{
            "name": request.json["name"],
            "last name": request.json["last_name"],
            "state": request.json["state"],
    ...
    def abort_if_not_exist(self,by,data):
        if by == "_id":
            response = database.db.Recycling.find_one({"_id":ObjectId(data)})
        else:
            response = database.db.Recycling.find_one({f"{by}": data})
